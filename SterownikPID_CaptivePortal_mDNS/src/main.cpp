...
#define AP_SSID "Sterownik_PID"
#define AP_PASS "config1234"  // adres do AP "pid.pl"
#define MDNS_NAME "pid" // adres do STA "pid.local"
#define DNS_PORT 53
...
// ============================================================
void setupWiFi();        // Ustawienie WiFi (AP lub STA)
void setupMDNS();        // Ustawienie mDNS
void checkResetButton(); // Sprawdzenie przycisku reset
void readTemperatures(); // Odczyt temperatur z czujników
void runPIDController(); // Sterowanie PID

void saveWiFiCredentials(); // Zapis danych WiFi do EEPROM
void loadWiFiCredentials(); // Odczyt danych WiFi z EEPROM
void saveSettings();        // Zapis ustawień PID do EEPROM
void loadSettings();        // Odczyt ustawień PID z EEPROM
void factoryReset();        // Przywrócenie ustawień fabrycznych

void handleAPConfig();      // Strona konfiguracji WiFi w trybie AP
void handleConnect();       // Obsługa formularza połączenia WiFi w trybie AP
void handleRoot();          // Strona główna w trybie STA
void handleAPI();           // API do odczytu danych
void handleSet();           // Ustawienie parametrów PID
void handleReset();         // Reset ustawień PID
void handleChart();         // Dane wykresu temperatur
void handleCaptivePortal(); // Obsługa żądań captive portal
void handleNotFound();      // Obsługa nieznanych żądań
...
{
    Serial.begin(115200);
    Serial.println(F("\n\n=== STEROWNIK PID (damian.podraza@gmail.com) ==="));
    ...
    if (isAPMode)
    {
        // Captive Portal - obsługa wszystkich popularnych endpointów
        ...
        // Catch-all dla pozostałych zapytań
...
void loop()
{
    // Obsługa DNS w trybie AP (Captive Portal)
    if (isAPMode)
    {
...
void setupWiFi()
{
    if (isAPMode)
    {
        ...
        // Uruchom serwer DNS dla Captive Portal
        ...
        Serial.println(F("DNS Server started - all domains redirect to AP"));
    }
    else
    {
        WiFi.mode(WIFI_STA);
...
void runPIDController()
{
    ...
    if (Ki > 0)
        pidIntegral = constrain(pidIntegral, 0, 100.0 / Ki);
...
void handleNotFound()
{
    // Sprawdź czy to żądanie do naszego IP
    ...
    // Jeśli host nie jest naszym IP - przekieruj (captive portal)
    ...
    // W przeciwnym razie pokaż stronę konfiguracji
    handleAPConfig();
}
...
// STRONA GŁÓWNA - CSS
// ============================================================
const char CSS_PART[] PROGMEM = R"(<!DOCTYPE html><html><head><meta charset='UTF-8'>
<meta name='viewport' content='width=device-width,initial-scale=1'>
...
// ============================================================
const char HTML_BODY[] PROGMEM = R"(<body><div class='c'>
<h1>&#127777; Sterownik PID</h1>
...
<div class='ft'>Sterownik PID (damian.podraza@gmail.com)</div></div>)";

// ============================================================
// STRONA GŁÓWNA - JAVASCRIPT
// ============================================================
const char JS_PART[] PROGMEM = R"(<script>
...
// ============================================================
// STRONA GŁÓWNA - wysyłana w częściach z PROGMEM
// ============================================================
void handleRoot()
{
